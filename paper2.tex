...
\] 
are random variables representing the \emph{phase noise} observed at the receiver~\cite{Tretter1985}.  We denote by $\angle{z}$ the complex argument of the complex number $z$, and by $\fracpart{x} = x - \round{x}$ the (centered) fractional part of the real number $x$, and by $\round{x}$ the nearest integer to $x$.  We have chosen to round half integers up.
...
\]
where $\expect$ denotes the expected value.  
...
\begin{align*}
\sigma^2 &= \expect\fracpart{\Phi -  \mu_{\text{intr}}}^2 = \expect\fracpart{\Phi}^2 \\
&= \expect \Phi^2 >  \expect (\Phi - \mu)^2 \geq  \expect \fracpart{\Phi - \mu}^2,
\end{align*}
violating the fact that $\mu_{\text{intr}} = 0$ is the minimiser of~\eqref{eq:intrmeandefn}.
\end{proof}
The next theorem describes the asymptotic distribution of the LSU estimator.  The theorem statement makes use of the function $\dealias(\cdot)$ that is defined in~\cite[Sec.~3]{McKilliam_pps1_2012}.
