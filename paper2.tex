...
Let $\widehat{\mubf}$ be defined by~\eqref{eq:hatmubfLSUdefn} and put $\widehat{\lambdabf}_N = \dealias(\tilde{\mubf} - \widehat{\mubf})$.  Denote the elements of $\widehat{\lambdabf}_N$ by $\widehat{\lambda}_{0,N}, \dots, \widehat{\lambda}_{m,N}$.  Suppose $\Phi_1, \dots, \Phi_N$ are independent and identically distributed with zero mean, variance $\sigma^2$, and pdf $f$ such that $f(\fracpart{x})$ is continuous at $x = -\nicefrac{1}{2}$ and $f(-\nicefrac{1}{2}) < 1$.  Suppose also that  $N^k \widehat{\lambda}_{k,N} \rightarrow 0$ in probability as $N\rightarrow\infty$ for all $k = 0, 1, \dots, m$.  Then the distribution of
\[
\left[
\begin{array}
[c]{cccc}%
...
Figure~\ref{plot:polyest1} shows the sample MSEs obtained for a polynomial phase signal of order $m=3$.  %Results are displayed results for the zeroth and third order coefficients $\widehat{\mu}_1$ and $\widehat{\mu}_3$.  The results for $\widehat{\mu}_1$ and $\widehat{\mu}_2$ lead to similar conclusions. 
